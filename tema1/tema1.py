...
def get_value(arg, input_title, type="string"):
    """
    arg: Valoare curenta
    input_title: Text input
    type: Tip variabila - int sau string
    returneaza: Valoare preluată din arg sau din input
    """
    if arg is not None:
        if type == "string":
            return arg
...
def main():
    ...
    algorithms = ["a_star", "a_star_opt", "ida_star"]
    heuristics = ["banala", "admisibila_1", "neadmisibila"]

    while True:
        try:
            algorithm = int(input("Tip algoritm (A* [1], A* Optimizat [2], IDA* [3], Toate[4]):\n"))
            heuristic = int(input("Tip euristică (Banală [1], Admisibilă 1 [2], Neadmisibilă [3], Toate[4]:\n"))
