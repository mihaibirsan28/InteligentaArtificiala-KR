from cerc import Graph, WalkingNode
...
def build_road(graph: Graph, current_node: WalkingNode, limit: int, timeout: float):
    """
    Construire drum
    graph: Graph(Graful nostru - cu detaliile problemei)
    current_node: WalkingNode(Nodul curent)
    limit: int(Limita de căutare)
    timeout: float(timpul maxim alocat)
    returneaza Solution(soluția găsită)
    """
...
def ida_star(graph, nsol, timeout):
    """
    IDA*
    graph: Graful nostru - cu detaliile problemei
    nsol: Numărul de soluții calculate
    timeout: Timpul limită alocat
    returneaza list[Solution(soluții găsite)]
    """
